...
pltdata.phylo.heatmap = pltdata.phylo.heatmap %>% dplyr::select(-tipLabelMatched) %>%
  dplyr::select(
    RTD,SRL,RD,RNC,SRA,RPC,RCC,RDMC, # FR traits
    LMA,LNC,LCC,LPC # leaf traits
    
  ) %>% # TRAITS
  dplyr::rename(
    R_RTD = RTD, R_SRL = SRL, R_RD = RD, R_RNC = RNC,
    R_SRA = SRA, R_RPC = RPC, R_RCC = RCC, R_RDMC = RDMC,
...
pltdata.phylo.heatmap.annot_traitkind = data.frame(
  TraitKind = rep(c("Fine Root","Leaf"), times = c(8,4))
)
...
ggsave(filename = "plt_TraitHeatmap_sel2503.pdf", plot = plt.phylo.total, width = 5.5, height = 6)
