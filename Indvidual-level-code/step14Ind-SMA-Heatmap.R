...
variables = c("LCC","LMA","LNC","LPC","Asat","Rdark25P","Vcmax25", # Leaf traits
              "RCC","RD","RDMC","RNC","RPC","RTD","SRA","SRL","SRR25") # Fine root traits
...
ggsave(plot=plt_SMA_htmap_pvalue, filename="2502-indv-level-code/Ind_SMAhtmap_Pvalue.pdf", width=6.4, height=6.4)
...
plt_SMA_htmap_rvalue = ggplot(data = melted_rvalue_matrix, aes(Var2, Var1, fill = value)) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 11), axis.text.y = element_text(size = 10)) +
  coord_fixed() +
  geom_text(aes(Var2, Var1, label = ifelse(abs(value)>0.25, value, " ")), color = "black", size = 4) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    panel.grid.major = element_blank(),
    ...
    legend.direction = "horizontal")+
  guides(fill = guide_colorbar(barwidth = 7, barheight = 1, title.position = "top", title.hjust = 0.5))
ggsave(plot=plt_SMA_htmap_rvalue, filename="2502-indv-level-code/Ind_SMAhtmap_Rvalue.pdf", width=6.4, height=6.4)
