...
traitDataIndv_spavg = traitDataIndv_spavg %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

traitDataIndv_spavg_log = traitDataIndv_spavg_log %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

traitDataIndv_spavg_log_ztransform = traitDataIndv_spavg_log_ztransform %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

# 3.2 - dataset normalized by sp and gf
traitDataIndv_spgfavg = traitDataIndv_spgfavg %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

traitDataIndv_spgfavg_log = traitDataIndv_spgfavg_log %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

traitDataIndv_spgfavg_log_ztransform = traitDataIndv_spgfavg_log_ztransform %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns

# 3.3 - the original, not species averaged dataset
traitDataIndv = traitDataIndv %>% rename(speciesFullName_ori = SpeciesFullName) %>%
  left_join(spTaxaNames_withfam, by = "speciesFullName_ori") %>%
  select(speciesFullName, family_name, genus_name, sp_name, everything()) # re-order columns
