...
nonphyloPCAData_numonly = traitDataPCA_touse %>% ungroup() %>% 
  dplyr::select(PCAIdentifier_spgf,GrowthForm, RTD,SRL,RD,RNC,LMA,LNC) %>% 
  na.omit()
...
plt_nonphyloPCA_biplot_ax12 = fviz_pca_biplot(
  ...
  annotate("text", x = -2.5, y = 3.8, label="cos2>0.3", color = "black") + 
...
plt_nonphyloPCA_biplot_ax23 = fviz_pca_biplot(
  ...
  # contrib: if contrib > 1, ex: 5, then the top 5 individuals/variables with the highest contrib are drawn
  addEllipses=T, palette = c("#8583A9", "#A0BDD5", "#CA8BA8")) + # liana, shrub, tree
  annotate("text", x = -2.8, y = 3.2, label="cos2>0.3", color = "black") + 
  labs(title = "") + theme_classic() + theme(legend.direction = 'horizontal', legend.position = 'none', legend.title = element_blank())
...
plt_ax23 = plt_nonphyloPCA_biplot_ax23 + plt_nonphyloPCA_contribplot_ax23 + 
  #plot_annotation(title = "PC2-3, biplot and contribution plot") +
  plot_layout(guides = "collect") & theme(legend.position='bottom')

# Combine the two plots side by side
plt_ax12_ax23_combined = plot_grid(plt_ax12, plt_ax23, ncol = 1)
ggsave(plot = plt_ax12_ax23_combined, filename = "plt_nonphyPCA_ax123Combined_RLES.pdf",
       width = 5, height = 7)
...
nonphyloPCA.screeplot = fviz_screeplot(nonphyloPCAresult, addlabels = TRUE, ylim = c(0, 40),
  ...
  labs(title="PCA, LES+RES") + theme_classic()
  ggsave(filename = "plt_nonphyPCA_scree-RLES.pdf", plot = nonphyloPCA.screeplot, width = 3, height = 3)

# Saving plot individually -----------------------------------------------------
# plt_multiply_fctr = 0.8
# ggsave(plot = plt_nonphyloPCA_biplot_ax12, filename = "plt_nonphyPCA_bi_RLESext_log_PC12.pdf", width = 4.5*plt_multiply_fctr, height = 4.7*plt_multiply_fctr)
# ggsave(plot = plt_nonphyloPCA_biplot_ax23, filename = "plt_nonphyPCA_bi_RLESext_log_PC23.pdf", width = 4.5*plt_multiply_fctr, height = 4.7*plt_multiply_fctr)
# 
# ggsave(plot = plt_nonphyloPCA_contribplot_ax12, filename = "plt_nonphyPCA_contrib_RLESext_log_PC12.pdf", width = 4.5*plt_multiply_fctr, height = 4.7*plt_multiply_fctr)
# ggsave(plot = plt_nonphyloPCA_contribplot_ax23, filename = "plt_nonphyPCA_contrib_RLESext_log_PC23.pdf", width = 4.5*plt_multiply_fctr, height = 4.7*plt_multiply_fctr)
